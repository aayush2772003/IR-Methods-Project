...
class ListwiseDataset(Dataset):
    def __init__(self, data):
        self.data = data
        self.vectorizer = TfidfVectorizer(max_features=10000)  # Limiting to the top 10000 features, actual number of features 26332
        # Prepare the corpus for vectorization by concatenating titles and abstracts
        all_docs = [title + " " + abstract for query, docs, rel_scores in data for title, abstract in docs]
    ...
    def __getitem__(self, idx):
        ...
        # Vectorize each document text
        doc_vectors = [self.vectorizer.transform([title + " " + abstract]).toarray() for title, abstract in docs]
        # Convert list of numpy arrays to a single numpy array then to a tensor
        docs_tensor = torch.tensor(np.stack(doc_vectors), dtype=torch.float)
        rel_scores_tensor = torch.tensor(rel_scores, dtype=torch.float)
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f'Using device: {device}')
model = ListNet(input_size=10000, hidden_size=512).to(device)
